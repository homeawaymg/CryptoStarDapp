...
it('can Create a Star', async() => {
    ...
    assert.equal(starInfo.symbol, 'AWS');
});
...
it('can add the star name and star symbol properly', async() => {
    ...
    let tokenId = 6;
    let instance = await StarNotary.deployed();
    await instance.createStar('Super Star!', 'SSS', tokenId, {from: accounts[1]});
    let starInfo = await instance.tokenIdToStarInfo.call(tokenId);
    //console.log(starInfo);
    assert.equal(starInfo.name, 'Super Star!');
    assert.equal(starInfo.symbol, 'SSS');
...
it('lets 2 users exchange stars', async() => {
    ...
    let tokenId2 = 8;
    let instance = await StarNotary.deployed();
    await instance.createStar('Exchange Star1', 'EX1', tokenId1, {from: accounts[0]});
    await instance.createStar('Exchange Star2', 'EX2', tokenId2, {from: accounts[1]});
...
it('lets a user transfer a star', async() => {
    ...
    let tokenId1 = 9;
    let instance = await StarNotary.deployed();
    await instance.createStar('Transfer Star3', 'EX3', tokenId1, {from: accounts[0]});
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let tokenId = 10;
    let instance = await StarNotary.deployed();
    await instance.createStar('Lookup Star', 'LUS', tokenId, {from: accounts[0]});
